...
docker run \
  --name vector-tiles-martin --network vector-tiles \
  -v $(pwd)/martin:/tmp \
  -p 4000:3000 \
  ghcr.io/maplibre/martin --config /tmp/config.yaml

docker run \
  --name vector-tiles-t-rex --network vector-tiles \
  -v $(pwd)/t-rex:/tmp \
  -p 9090:9090 \
  sourcepole/t-rex:v0.14.3 --config /tmp/config.toml

docker run \
  --name vector-tiles-tegola --network vector-tiles \
  -v $(pwd)/tegola:/tmp \
  -p 7070:7070 \
